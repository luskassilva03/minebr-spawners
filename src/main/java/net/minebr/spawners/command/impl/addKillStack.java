...
public class addKillStack implements ISubCommand {
    @Override
    public void execute(CommandSender s, String[] a) {
        if (s.hasPermission("minegeradores.admin")) {
            ...
            double amountPlayer = playerManage.getQuantidade();
            double amountCommand = Double.parseDouble(amount);

            amountCommand += amountPlayer;
            playerManage.setQuantidade(amountCommand);
            s.sendMessage("§cKill Stack §7▸ §fVocê adicionou §c§l" + NumberFormat.numberFormat(Double.parseDouble(amount)) + "§cx Kill Stack §fpara este jogador§f.");
            s.sendMessage("             §7↳ §fNovo saldo é de §c§l" + NumberFormat.numberFormat(playerManage.getQuantidade()) + "§cx KillStack§f.");
            targetPlayer.sendMessage("§cKill Stack §7▸ §fFoi adicionado em sua conta §c§l" + NumberFormat.numberFormat(Double.parseDouble(amount)) + "§cx Kill Stack§f.");
